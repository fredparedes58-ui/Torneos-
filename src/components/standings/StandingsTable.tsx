    ...
    <div className="overflow-x-auto rounded-xl border border-[#7A8A55] bg-[#191D10]">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[#7A8A55] bg-[#282C1D]">
            ...
            {cols.map(c => (
              <th key={c.key} className={['text-center px-3 py-3 label-caps', c.key === 'pts' ? 'text-[#D4FF1F]' : 'text-[#A5B084]'].join(' ')}>
                {c.label}
              </th>
            ))}
          </tr>
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-white group-hover:text-[#D4FF1F] transition-colors whitespace-nowrap">
                      {row.equipo.nombre}
                    </span>
                  </div>
                </td>
                {cols.map(c => (
                  <td key={c.key} className={[
                    ...
                    c.key === 'pts' ? 'font-bold text-[#D4FF1F]' :
      ...
      <div className="flex gap-4 px-4 py-3 border-t border-[#7A8A55] bg-[#282C1D]">
        <span className="flex items-center gap-2 text-[10px] text-[#A5B084] font-mono">
          <span className="w-3 h-3 rounded-sm bg-[#2A3500] border border-[#D4FF1F60]" /> Clasificacion
        </span>
        <span className="flex items-center gap-2 text-[10px] text-[#A5B084] font-mono">
