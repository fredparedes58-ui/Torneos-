...
function MatchRow({ partido, delay }: { partido: Partido; delay: number }) {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 px-5 py-3 border-b border-[#434933] last:border-0 hover:bg-[#282C1D] transition-colors"
    >
      <div className="w-14 shrink-0 text-right">
        <p className="font-mono text-[10px] text-[#C4CAAC]">{partido.hora}</p>
        <p className="font-mono text-[9px] text-[#8E9479]">{partido.fecha.slice(5)}</p>
      </div>

      <div className="flex items-center gap-2 flex-1 justify-end">
        <span className={['text-sm font-medium truncate max-w-28', localGana ? 'text-[#C8FF00]' : 'text-white'].join(' ')}>
      ...
      <div className="flex items-center gap-1 shrink-0">
        {isJugado ? (
          <span className={[
            ...
            empate
              ? 'text-[#FFB800] bg-[#FFB80010] border-[#FFB80040]'
              : 'text-white bg-[#0C0F04] border-[#434933]',
          ].join(' ')}>
            {partido.golesLocal} – {partido.golesVisitante}
          </span>
        ) : (
          <span className="font-mono text-sm text-[#8E9479] px-3 py-1 rounded-lg border border-[#434933] bg-[#191D10]">
            VS
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 flex-1">
        ...
        <span className={['text-sm font-medium truncate max-w-28', visitGana ? 'text-[#C8FF00]' : 'text-white'].join(' ')}>
  ...
  return (
    <div className="flex flex-col gap-4">
      {groups.map(([jornada, ps], gi) => (
        <div key={jornada} className="rounded-xl border border-[#434933] bg-[#191D10] overflow-hidden">
          <div className="px-5 py-3 border-b border-[#434933] bg-[#282C1D] flex items-center gap-3">
            <span className="font-display font-bold text-sm uppercase tracking-widest text-[#C4CAAC]">
              {ps[0].fase ? ps[0].fase : `Jornada ${jornada}`}
            </span>
            <span className="text-[10px] font-mono text-[#8E9479]">
